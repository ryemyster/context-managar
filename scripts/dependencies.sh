#!/usr/bin/env bash
# Usage: bash scripts/dependencies.sh owner/repo/path [summary|full] [max_results]
# Example: bash scripts/dependencies.sh ryemyster/context-manager/context-engine/app
# Default mode is context-safe summary references. Pass "full" only when needed.

PATH_ARG="${1:?Usage: bash scripts/dependencies.sh owner/repo/path [summary|full] [max_results]}"
DETAIL="${2:-summary}"
MAX_RESULTS="${3:-20}"
