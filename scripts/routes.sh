#!/usr/bin/env bash
# Usage: bash scripts/routes.sh owner/repo/path [summary|full] [max_results]
# Example: bash scripts/routes.sh ryemyster/context-manager
# Default mode is context-safe summary references. Pass "full" only when needed.

PATH_ARG="${1:?Usage: bash scripts/routes.sh owner/repo/path [summary|full] [max_results]}"
DETAIL="${2:-summary}"
MAX_RESULTS="${3:-20}"
BASE_URL="${CONTEXT_ENGINE_URL:-http://localhost:8088}"

echo "→ Extracting routes in: $PATH_ARG ..."
echo ""
...
echo ""
echo "Artifact path is in written_to. Pass full for legacy inline route lists."
