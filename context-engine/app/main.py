...
async def setup():
    ...
    return (
        ...
        "- `final_answer`, `tool_calls_made` `{name, arguments, result}`, `iterations`, `stopped_reason`\n"
        ...
        "- `verification` — post-hoc coherence check: `{passed: bool|null, rationale: str, unsupported_claims: [str], evidence_gap: bool}`;\n"
        "  populated only when `stopped_reason == \"final_answer\"`; `{}` on timeout/max_iterations/error\n\n"
        ...
        "Advisory only — the answer is always returned regardless of verdict. "
        "Skipped on `max_iterations`, `timeout`, or `model_error` stops.\n\n"
