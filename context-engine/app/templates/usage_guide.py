...
def get_usage_guide(
    ...
    return (
        ...
        "- **Internal Tool Allowlist**: `investigate_codebase`'s optional `tools` argument allowlists *internal* "
        "ReAct-loop tools, not MCP tool names. Valid values: `scan_directory`, `find_in_code`, `read_file`, `grep`, "
        "`health_check`, `search_memory`, `update_plan` (see `GET /agents/tools` for schemas). Do not pass MCP tool "
        "names like `summarize_file`, `vector_search`, or `investigate_codebase` itself. Leave `tools` empty to enable "
        "all internal tools; unrecognized entries are dropped with a warning in the response rather than failing the run.\n\n"
        ...
        f"| `GET /agents/tools` | Interactive | — | Lists valid internal `tools` allowlist names and schemas for `/agents/run` |\n\n"
