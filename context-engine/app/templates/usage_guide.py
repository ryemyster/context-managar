...
def get_usage_guide(
    ...
    return (
        ...
        "- **MCP Delegation Contract**: This service is an MCP-connected junior engineering assistant. Prefer the "
        "`investigate_codebase` MCP tool for delegated repository work; use direct REST endpoints only for bounded "
        "primitive retrieval or service operations.\n"
        ...
        "- **Required Evidence**: When a delegation must inspect specific source files, pass their exact paths in "
        "`required_paths`. A report is `verified` only after every required path was successfully read; inspect broader "
        "directories and symbols through the task text.\n"
        ...
        "1. **Delegate**: For open-ended repository investigation, call the MCP `investigate_codebase` tool with a bounded "
        "task, exact must-read files in `required_paths`, and enough iterations for discovery, reading, and synthesis. "
        "The MCP adapter owns REST start/poll mechanics. If the task is "
        ...
        "- Treat an agent report as conclusive only when `status: verified`, `verification.passed: true`, and "
        "`evidence_coverage.complete: true`. `partial`, `blocked`, and `failed` are work reports for follow-up, not "
        "answers to act on.\n"
        "- On `partial`, use `evidence_coverage.missing_paths` and the returned tool evidence to narrow a retry; do not "
        "ask the agent to infer unread files.\n"
